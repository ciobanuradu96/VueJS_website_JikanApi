<template>
  <div>
    ...
    <carousel
      class="flex flex-row items-center w-10/12 mx-auto bg-transparent h-80"
      :loop="true"
      :autoplay="true"
    ...
    >
      <slide v-for="(results, index) in data" :key="index" class="">
        <router-link :to="`/anime/${data[index].mal_id}`">
          <div
            class="mx-auto relative bg-no-repeat bg-center bg-cover h-72 w-48 m-1 border-2 transform hover:scale-105"
